<template>
  <div>
    ...
    <div>
      ...
      <button @click="addTodo" @keyup.enter="addTodo">추가</button>
    </div>
    <br />
    <!-- 리스트 파트 -->
    <ol>
      ...
      <li v-for="todoItem in states.todoList" :key="todoItem.id">
        ...
        <div v-if="todoItem.editing">
          ...
          <button @click.stop="cancleEditTodo(todoItem.id)">취소</button>
        </div>
        &nbsp;
...
async function addTodo() {
  if (todo.value === '') return alert('할 일 입력해라');

  try {
    const newTodo = { todo: todo.value, done: false };
    const addTodoRes = await axios.post(BASEURL, newTodo);

    if (addTodoRes.status !== 201) return alert('Todo 추가하지마');
    todo.value = '';
    fetchTodoList();
  } catch (error) {
    alert('Todolist 추가 작업중, ERR 발생');
    console.log(error);
  }
}
...
async function deleteTodo(id) {
  try {
    ...
    if (deleteTodoRes.status !== 200) return alert('Todo 삭제실패');
    fetchTodoList();
  } catch (err) {
...
function cancleEditTodo(id) {
  const targetTodo = states.todoList.find((todo) => todo.id === id);
  targetTodo.editing = false;
}

async function confirmEditTodo(id) {
  if (todoEdit.value === '') return alert('수정 내용을 입력해');
  try {
    const targetTodo = states.todoList.find((todo) => todo.id === id);
